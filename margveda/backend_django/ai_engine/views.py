...
class PublicChatView(APIView):
    ...
    Routes through CareerBrain → CareerAgent → AI Gateway.
    """
    permission_classes = []
    ...
    def post(self, request):
        ...
        from apps.ai.careerbrain import CareerBrain, BrainRequest
        brain = CareerBrain.get()

        result = brain.process(BrainRequest(
            query=message,
            session_id=request.data.get("session_id"),
            agent_hint=request.data.get("agent", "career"),
...
class ChatView(APIView):
    """
    POST /api/v1/ai/chat/
    Authenticated endpoint — routes through CareerBrain with student memory.
    """
    permission_classes = [IsAuthenticated]
    ...
    def post(self, request):
        ...
        return Response({
            ...
            "citations": result.citations,
            "needs_counselor": result.needs_counselor,
            "counselor_reason": result.metadata.get("counselor_reason", ""),
            "agent_used": result.agent_used,
