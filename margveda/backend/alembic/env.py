...
Reads DATABASE_URL from the application settings (which loads .env automatically),
imports all SQLAlchemy models so their tables appear in metadata, then runs
offline or online migrations.
"""
...
# alembic/env.py lives one level inside the backend root, so we add the
